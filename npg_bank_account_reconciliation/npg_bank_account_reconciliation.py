...
class bank_acc_rec_statement(osv.osv):
    def check_group(self, cr, uid, ids, context=None):
        ...
        Bank Statement Preparer– they can create, view and delete any of the
            Bank Statements provided the Bank Statement is not in the DONE
            state, or the Ready for Review state.
        Bank Statement Verifier – they can create, view, edit, and delete any
            of the Bank Statements information at any time.  NOTE: DONE Bank
            Statements  are only allowed to be deleted by a Bank Statement
            Verifier."""
        model_data_obj = self.pool.get('ir.model.data')
        res_groups_obj = self.pool.get('res.groups')
        group_verifier_id = model_data_obj._get_id(
            cr, uid,
            'npg_bank_account_reconciliation', 'group_bank_stmt_verifier')
            ...
            if group_verifier_id:
                ...
                if statement.state != 'draft' and uid not in group_user_ids:
                    raise orm.except_orm(
                        _('User Error !'),
                        _("Only a member of '%s' group may delete/edit bank "
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        res = super(bank_acc_rec_statement, self).write(cr, uid, ids, vals, context=context)
    ...
    def check_difference_balance(self, cr, uid, ids, context=None):
        ...
        for stmt in self.browse(cr, uid, ids, context=context):
            if stmt.difference != 0.0 and not stmt.multi_currency:
                raise orm.except_orm(
                    _('Warning'),
            ...
            elif stmt.difference_in_currency != 0.0 and stmt.multi_currency:
                raise orm.except_orm(
                    _('Warning'),
            ...
            elif stmt.multi_currency and (
                stmt.general_ledger_balance != stmt.registered_balance):
                raise orm.except_orm(
                    _('Warning'),
                    _('Prior to reconciling a statement in a foreign currency'
    ...
    def onchange_account_id(self, cr, uid, ids, account_id, ending_date,
        ...
        account_obj = self.pool['account.account']
        account_move_line_obj = self.pool['account.move.line']
        statement_line_obj = self.pool['bank.acc.rec.statement.line']
        val = {'value': {
            'credit_move_line_ids': [], 'debit_move_line_ids': [],
    ...
    _columns = {
        ...
        'currency_rate': fields.float('Currency Exchange Rate',
                                      digits=(12,6)),
        ...
        'exchange_date': fields.date(
            'Currency Exchange Date', required=False,
            states={'done':[('readonly', True)]},
        ...
        'starting_balance_in_currency': fields.float(
            'Starting Balance in Currency', required=True,
            digits_compute=dp.get_precision('Account'),
        ...
        'ending_balance_in_currency': fields.float(
            'Ending Balance in Currency', required=True,
            digits_compute=dp.get_precision('Account'),
        ...
        'uncleared_balance': fields.function(
            ...
            digits_compute=dp.get_precision('Account'),
            help="Total Sum of the Deposit Amount not Cleared – Total Sum of "
                 "Checks, Withdrawals, Debits, and Service Charges Amount "
                 "not Cleared",
            multi="balance"),
...
class bank_acc_rec_statement_line(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not vals.get('move_line_id', False):
            raise orm.except_orm(_('Processing Error'),_('You cannot add any new bank statement line manually as of this revision!'))
